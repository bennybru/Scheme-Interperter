...
import { Value, isSymbolSExp, isCompoundSExp, makeCompoundSExp, makeEmptySExp, isEmptySExp, CompoundSExp, EmptySExp, isTuple, Tuple, makeTuple } from "./L5-value";
...
const isPairPrim = (v: Value): boolean =>
    ...
    const valuesPrim = (args: Value[]): Result<Tuple> => 
        args.reduce((acc: boolean, cur: Value) => isTuple(cur) || acc, false) ? 
            makeFailure("No support for tuple inside a tuple") : 
                makeOk(makeTuple(args))
