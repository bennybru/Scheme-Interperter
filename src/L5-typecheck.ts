...
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         ...
         Parsed, PrimOp, ProcExp, Program, StrExp, isLetValuesExp, CExp, LetValuesExp, ValuesBinding, VarDecl } from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, isTupleTExp, isEmptyTupleTExp, makeEmptyTupleTExp, makeNonEmptyTupleTExp, isNonEmptyTupleTExp, isNonTupleTExp } from "./TExp";
...
export const typeofApp = (app: AppExp, tenv: TEnv): Result<TExp> =>
    isPrimOp(app.rator) && app.rator.op === "values" ?           
        bind(mapResult((cexp: CExp) => typeofExp(cexp, tenv), app.rands), (texps: TExp[]) =>
            isEmpty(texps) ? makeOk(makeEmptyTupleTExp()) :
                allT(isNonTupleTExp, texps) ? 
                    makeOk(makeNonEmptyTupleTExp(texps)) :
                        makeFailure("Invalid types")
        ) :
...
export const typeofLetvalues = (exp: LetValuesExp, tenv: TEnv): Result<TExp> => {
    const vars = exp.bindings.map((binding) => binding.vars.map((v: VarDecl) => v.var)).reduce((acc, cur) => acc.concat(cur),[]);
    const vals = exp.bindings.reduce((acc: CExp[], cur) => 
        isAppExp(cur.val) && isPrimOp(cur.val.rator) && cur.val.rator.op === 'values' ? 
            acc.concat(cur.val.rands) : acc.concat(cur.val) ,[]);
    const varTEs = exp.bindings.map((binding) => binding.vars.map((v: VarDecl) => v.texp)).reduce((acc, cur) => acc.concat(cur),[]);
...
// (let-values ( ( (x: number z: number) (values 1 2) ) ) )
