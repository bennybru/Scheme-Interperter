...
const evalLetValues = (exp: LetValuesExp, env: Env): Result<Value> => {
    const vals = exp.bindings.map((binding: ValuesBinding) => binding.val);
    const tupleVals = bind(
        mapResult((v: CExp) => applicativeEval(v, env), vals), 
        (vals: SExpValue[]) => 
            mapResult(
                (val: SExpValue): Result<SExpValue[]> => 
                    isTuple(val) ? makeOk(val.vals) : 
                    makeFailure('Invalid type'), 
                vals));

    return safe2((vars: VarDecl[][], vals: SExpValue[][]): Result<Value> => 
        bind(
            ((vars: VarDecl[][], vals: SExpValue[][]): Result<boolean[]> => 
                mapResult((vardeclArr: VarDecl[]) =>
                    vardeclArr.length === vals[vars.indexOf(vardeclArr)].length ?
                        makeOk(true) : 
                        makeFailure("sizes differ!"), 
                vars))(vars, vals), 
            () =>
                evalSequence(
                    exp.body, 
                    makeExtEnv(
                        vars.reduce((acc: VarDecl[], cur: VarDecl[]): VarDecl[] => acc.concat(cur),[]).map((v: VarDecl) => v.var),
                        vals.reduce((acc: SExpValue[], cur: Value[]): Value[] => acc.concat(cur),[]),
                        env))))
    (makeOk(exp.bindings.map((binding: ValuesBinding) => binding.vars)), tupleVals);
}