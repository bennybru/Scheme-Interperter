...
//Question2: added <vals-binding>,
// extended <cexp> to support <let-values>, 
// extended <prim-op> to support values
...
//         |  ( let-values ( <vals-binding>* ) <cexp>+ )  / LetValuesExp(bindings:ValuesBinding[], body:CExp[]))
// <vals-binding>  ::= ( ( <var-decl>* ) <cexp> ) / ValuesBinding(vars:VarDecl[], val:Cexp)
...
//Question2: extended CompoundExp to support LetValuesExp
...
//Question2: added ValuesBinding type, constructor and type-predicat
...
//Question2: added LetValuesExp type, constructor and type-predicat
...
//Question2: extended the following function to support LetValuesExp
...
const isSpecialForm = (x: string): boolean =>
    ["if", "lambda", "let", "let-values", "quote", "letrec", "set!"].includes(x);
...
const isGoodValuesBindings = (bindings: Sexp): bindings is [Sexp[], Sexp][] =>
    isArray(bindings) && allT(isArray, bindings) && allT(isArray, bindings.map(first));
...
const parseValuesBindings = (bindings: [Sexp[], Sexp][]): Result<ValuesBinding[]> =>
    safe2((vds: VarDecl[][], vals: CExp[]) => makeOk(zipWith(makeValuesBinding, vds, vals)))
        (mapResult((sexps: Sexp[]): Result<VarDecl[]> =>
            mapResult(parseVarDecl, sexps), 
        map(b => b[0], bindings)), mapResult(parseL5CExp, map(b => b[1], bindings)));
...
const unparseValuesBindings = (bindings: ValuesBinding[]): Result<string> =>
    bind(mapResult(bdg => safe2((vds: string, val: string) => makeOk(`((${vds}) ${val})`))
        (((vds: VarDecl[]): Result<string> =>
            bind(mapResult(unparseVarDecl, vds), parsed => 
                makeOk(join(" ", parsed))))    
        (bdg.vars), unparse(bdg.val)), bindings),
        (bdgs: string[]) => makeOk(join(" ", bdgs)));
...
const unparseLetValuesExp = (lve: LetValuesExp) : Result<string> => 
    safe2((bdgs: string, body: string) => makeOk(`(let-values (${bdgs}) ${body})`))
        (unparseValuesBindings(lve.bindings), unparseLExps(lve.body));